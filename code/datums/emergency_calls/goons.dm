...
/datum/emergency_call/goon/bodyguard/print_backstory(mob/living/carbon/human/M)
	...
	to_chat(M, role_body("Joining the ranks of Weyland-Yutani was all you could do to keep yourself and your loved ones fed."))
	to_chat(M, role_body("You have no idea what a xenomorph is."))
	to_chat(M, role_body("You are a simple security officer employed by Weyland-Yutani to guard their Executives from all Divisions alike."))
	to_chat(M, role_body("You were sent to act as the Executives bodyguard on the [MAIN_SHIP_NAME], you have gotten permission from corporate to enter the area."))
	to_chat(M, role_body("Ensure no damage is incurred against Weyland-Yutani. Make sure the CL is safe."))
